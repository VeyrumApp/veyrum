...
test("a project's own coverage session and capture each see every execution since their last take", async () => {
...
test('a mode change under a consumer is reported to it: V8 stops reporting older functions', async () => {
  const hub = coverageHub()
  const capture = {}
