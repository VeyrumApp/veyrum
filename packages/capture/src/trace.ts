 ...
 * same log. On Windows, every program starts through a launcher (native/exec-win.c) that loads a
 * DLL (native/trace-win.c) into it, which writes the same log for it and everything it starts.
 * This module decides how a program runs and parses the log. Everything here runs with the capture
...
export const TRACE_LIBRARY = path.join(NATIVE_DIR, WINDOWS ? 'veyrum-trace.dll' : 'libveyrum-trace.so')
...
/**
 * The launcher that traces statically linked and Go programs with ptrace, or on Windows starts
 * every program with the DLL; built with the library.
 */
...
export const TRACED_PLATFORMS: readonly string[] = ['linux-x64', 'linux-arm64', 'win32-x64']
 ...
 * creates the log, so a trivial traced program shows whether it loaded. On Windows the launcher
